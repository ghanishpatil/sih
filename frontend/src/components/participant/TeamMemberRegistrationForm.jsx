  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      for (const member of members) {
        ...
        if (!data) {
          throw new Error(`Missing data for ${member.displayName}`)
        }
        
        if (!data.name?.trim()) {
          throw new Error(`Name is required for ${member.displayName}`)
        }
        
        if (!data.institute?.trim()) {
          throw new Error(`Institute is required for ${member.displayName}`)
        }
        
        if (!data.email?.trim()) {
          throw new Error(`Email is required for ${member.displayName}`)
        }
        
        // Email format validation
        const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/
        if (!emailRegex.test(data.email.trim())) {
          throw new Error(`Invalid email format for ${member.displayName}`)
        }
        
        if (!data.phone?.trim()) {
          throw new Error(`Phone number is required for ${member.displayName}`)
        }
        
        // Phone validation (10 digits)
        const phoneDigits = data.phone.trim().replace(/\D/g, '')
        if (phoneDigits.length !== 10) {
          throw new Error(`Phone number must be 10 digits for ${member.displayName}`)
        }
        
        if (!data.idCard) {
          throw new Error(`ID card PDF is required for ${member.displayName}`)
        }
      }

      // Submit each member's registration sequentially
      for (const member of members) {
        const data = formData[member.uid]
        ...
        formDataToSend.append('email', data.email.trim().toLowerCase())
        formDataToSend.append('phone', data.phone.trim().replace(/\D/g, ''))
        ...
        if (!response.ok) {
          ...
          throw new Error(err.error || `Failed to submit registration for ${member.displayName}`)
        }
      }
        ...
        {success && (
          <div className="rounded-lg bg-green-50 border border-green-200 px-4 py-3 text-sm text-green-800">
            <div className="flex items-center gap-2">
              ...
              <span>All member registrations submitted successfully!</span>
            </div>
          </div>
        )}
